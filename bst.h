/*************************************************************************
	> File Name: bst.h
	> Author: Sherlock_Xujianguo
	> Mail: 1397443527@qq.com
	> Created Time: Fri 19 Jul 2019 06:34:34 PM CST
 ************************************************************************/
...
namespace toy {
    ...
    private:
        ...
        void eraseSingleNode(BasePtr node) {
            ...
            if (node->left == nullptr) {
                if (is_left_child) insert_left(parent, node->right);
                else insert_right(parent, node->right);
            } else if (node->right == nullptr) {
                if (is_left_child) insert_left(parent, node->left);
                else insert_right(parent, node->left);
            } else {
                if (is_left_child) insert_left(parent, node->right);
                else insert_right(parent, node->right);
    ...
    public:
        ...
        iterator begin() {
            if (nullptr == root()) return &header;
            return BinarySearchTreeHelper::leftMostNode(root());
        }

        const_iterator begin() const {
            if (nullptr == root()) return const_iterator(&header);
            return root();
        }
